...
function Labitem({ labLevel, scenario, solved }) {
  const navigate = useNavigate();
  const [status, setStatus] = useState(solved ? "Solved" : "Solve");

  const handleRightClick = (e) => {
    e.preventDefault();
    navigate("/fulllab", { state: { labLevel, scenario, currentStatus: status } });
    if (status === "Solve") {
      setStatus("Opened");
    }
  };
  ...
  return (
    <div className="flex flex-col md:flex-row items-stretch w-full mb-4 overflow-hidden rounded-full shadow-lg mr-8">
      ...
      <div
        ...
        className="cursor-pointer bg-[#5DB717] md:w-[20%] flex items-center justify-center md:rounded-r-full p-4"
      >
